...
//            Set color and size based on specific joint name.
//            GREEN joints are actively tracked by ARKit. Yellow joints are not tracked -> they follow the motion of the closest green joint.
            switch jointName {
...
//            Create an entity for the joint, add to joints directory and add it to the parent entity -> bodySkeleton
...
//            set orientation for bone
    ...
    private func createJoint(radius: Float, color: UIColor = .white) -> Entity {
        ...
        let material = SimpleMaterial(color: color, roughness: 0.0, isMetallic: false)
...
//        relative to root/hip joint
        let jointFromEntifyPosition = jointFromEntityOffsetFromRoot + rootPosition
...
//        realtive to world reference frame
        
        let fromJoint = SkeletonJoint(name: bone.jointFromName, position: jointFromEntifyPosition)
